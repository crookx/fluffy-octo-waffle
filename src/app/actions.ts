...
// Internal helper to compute post-login redirect for the current session
async function handleLoginRedirectAction(redirectUrl?: string | null): Promise<string> {
  console.log('[Server] handleLoginRedirectAction called with:', redirectUrl);
  ...
  if (redirectUrl) {
    try {
      ...
      console.error('[Server] Error parsing redirect URL:', err);
    }
  }

  console.log('[Server] Returning redirect:', finalRedirect);
